...
using face_features_map_t = std::unordered_map<
        std::string,
        face_features_t
>;
...
template<
        template<
        int, template<typename> class, int, typename
> class block, int N,
...
class FaceRecognizer {
 public:
    ...
    */
    float GetSimilarityFromImg(const std::string &face_feature1,
    ...
    * @param break_if_first_matched A boolean indicating whether to stop searching
    * for matches after the first match is found.
    * @return An integer indicating the number of matches found.
    */
    int RecognizeImgByKnownFaces(const std::string &image_path,
                                    std::string* name,
                                    bool break_if_first_matched = true);
    ...
    */
    void AddKnownFaceFeatures(std::string known_face_name,
                                face_features_t face_features);
    ...
    */
    void AddKnownFaceFeatures(std::string known_face_name,
                                const std::string &face_features);
    ...
    */
    // TODO(nevermore) this func spent most time, it takes 4~500ms
    std::vector <face_features_t>
    GetQueryFaceFeaturesFromImg(const std::string &image_path);
    ...
    */
    float GetSimilarity(const std::string &face_feature_str,
                        std::vector <face_features_t> query_face_features);
    ...
    * @return The similarity score between the face feature and the query face features, as a float.
    */
    float GetSimilarity(const face_features_t &face_feature,
                        std::vector <face_features_t> query_face_features);
    ...
    */
    bool Match(const std::string &face_feature_str,
                std::vector <face_features_t> query_face_features);
    ...
    * @return A boolean indicating whether a match was found.
    */
    bool Match(const face_features_t &face_feature,
                std::vector <face_features_t> query_face_features);
 ...
 public:
    ...
    *
    * This function decodes a face feature vector from a string
    * that was encoded using the EncodeFaceFeature function.
    */
    static face_features_t
    DecodeFaceFeature(const std::string &face_features_str);
    ...
    */
    static inline bool IsMatched(float sim) {
        return sim < matched_max_threshold;
    }
 ...
 protected:
    dlib::shape_predictor &GetLandmarkDetector() {
        std::call_once(sp_init_flag_, &FaceRecognizer::InitLandmarkDetector,
                        this);
        return sp_;
    }

    anet_type &GetFaceRecognizer() {
        std::call_once(net_init_flag_, &FaceRecognizer::InitFaceRecognizer,
                        this);
        return net_;
    }
 ...
 private:
    ...
    // "C:/Users/SG220/source/repos/testCmakeProj/shape_predictor_68_face_landmarks.dat";
    // std::string face_recognition_resnet_dat =
    // "C:/Users/SG220/source/repos/testCmakeProj/dlib_face_recognition_resnet_model_v1.dat";
