...
float
FaceRecognizer::GetSimilarity(const std::string &face_feature_str, std::vector <face_features_t> query_face_features) {
...
float
FaceRecognizer::GetSimilarity(const face_features_t &face_feature, std::vector <face_features_t> query_face_features) {
    float min_distance = std::numeric_limits<float>::max();
    // Initialize the face detector
    dlib::frontal_face_detector detector = dlib::get_frontal_face_detector();
...
bool FaceRecognizer::Match(const face_features_t &face_feature, std::vector <face_features_t> query_face_features) {
...
int FaceRecognizer::RecognizeImgByKnownFaces(const std::string &image_path, std::string *name,
                                             bool break_if_first_matched) {
